...
/**
 * get_int - get char pointer to new string with int
 * @num: number toconvert to string
 * Return: char pointer converted as null
 * if fails and malloc.
 */
 ...
 * Return: Nothing
 */
void full_buff(unsigned int num, unsigned int base, char *buff,
		int buff_size)
{
 ...
 *
 * Return:if it is a digit, return 0. otherwise 0.
 */
int _isdigit(void)
{
	int i = 0;
	...
	{
		if ((globalvar.token2[i] >= '0' &&
					(globalvar.token2[i + 1] >= '0'
		|| globalvar.token2[i + 1] == '\0')) &&
				(globalvar.token2[i] <= '9' &&
				 (globalvar.token2[i + 1] <= '9'
				  || globalvar.token2
					 [i + 1] == '\0')))
			return (1);
		else
